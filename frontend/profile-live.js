(() => {
  ...
  function render(user) {
    ...
    fetch((window.ERIS_API||window.ERISCHAT_API||'https://erischat-production.up.railway.app/v1')+'/me/vip',{headers:(()=>{const t=localStorage.getItem('erischat_access_token')||localStorage.getItem('erischat.accessToken.v1')||localStorage.getItem('token')||'';return t?{Authorization:'Bearer '+t}:{}})()}).then(r=>r.ok?r.json():null).then(v=>{if(!v)return;const l=document.getElementById('profileVipLevel'),b=document.getElementById('profileVipBadge'),bar=document.querySelector('.profile .progress .bar i'),pct=document.querySelector('.profile .progressTop span:last-child');const level=Number(v.level||0);if(l)l.textContent=String(level);if(b)b.textContent=level?'VIP '+level:'VIP';if(v.total_spent!=null){const spent=Number(v.total_spent||0);const thresholds=[0,1000,5000,15000,30000,60000,120000,250000,500000,1000000,2000000,5000000,10000000];const next=thresholds[Math.min(level+1,12)]||spent;const prev=thresholds[Math.min(level,12)]||0;const ratio=level>=12?1:Math.max(0,Math.min(1,(spent-prev)/Math.max(1,next-prev)));if(bar)bar.style.width=(ratio*100)+'%';if(pct)pct.textContent=level>=12?'MAX':Math.round(ratio*100)+'%';}}).catch(()=>{});
  }
