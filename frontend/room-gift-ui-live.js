...
(() => {
  const state = { roomId: null, gifts: [], recipients: [], selectedGift: null, selectedRecipient: null, ready: false };
...
';
  ...
  function ensureUi() {
    if (document.getElementById('erischatGiftFab')) return;
    injectStyles();
  ...
  function renderRecipients() {
    const box = document.getElementById('egpRecipients'); if (!box) return;
    box.innerHTML = state.recipients.length ? state.recipients.map((r, i) => `<button class="egp-chip${state.selectedRecipient===r.id?' active':''}" data-rec="${esc(r.id)}" type="button">👤 ${esc(r.name)}</button>`).join('') : '<div class="egp-note">Şu anda hediye gönderilebilecek aktif koltuk bulunamadı.</div>';
  ...
  function renderGifts() {
    const box = document.getElementById('egpGifts'); if (!box) return;
  ...
  async function refresh() {
    ...
    try {
      ...
      const balance = document.getElementById('egpBalance'); if (balance) balance.textContent = `💎 ${Number(me.lidya || 0).toLocaleString('tr-TR')}`;
  ...
  function updateSend() {
    const button = document.getElementById('egpSend'); if (!button) return;
    button.disabled = !(state.roomId && state.selectedRecipient && state.selectedGift);
  }

  async function send() {
    if (!state.roomId || !state.selectedRecipient || !state.selectedGift) return;
    const button = document.getElementById('egpSend'); button.disabled = true; button.textContent = 'Gönderiliyor…';
    try {
      ...
      window.ErisPlatform.getMe().then(me => { const balance = document.getElementById('egpBalance'); if (balance) balance.textContent = `💎 ${Number(me.lidya || 0).toLocaleString('tr-TR')}`; }).catch(() => {});
    } catch (error) { toastSafe(error.message || 'Hediye gönderilemedi.'); }
    finally { button.textContent = 'Hediye gönder'; updateSend(); }
  }

  function setRoom(roomId, open = false) {
    state.roomId = roomId ? String(roomId) : null;
    const fab = document.getElementById('erischatGiftFab'); if (fab) fab.style.display = state.roomId ? 'block' : 'none';
    if (state.roomId && open) { ensureUi(); refresh(); }
  }
