...
(() => {
  ...
  function renderGifts() {
    ...
    if (!box) return;
    if(cats) cats.innerHTML=GIFT_CATEGORIES.map(x=>`<button class="egp-cat${state.category===x[0]?' active':''}" data-cat="${esc(x[0])}" type="button">${esc(x[1])}</button>`).join('');
  ...
  function ensureUi() {
    ...
    panel.innerHTML = '<div class="egp-head"><div><div class="egp-title">🎁 Odaya hediye gönder</div><div class="egp-balance" id="egpBalance">Bakiye yükleniyor…</div></div><button class="egp-close" type="button">×</button></div><div class="egp-note">Alıcıyı seç; kategoriyi aç; hediyeyi yalnızca logosundan seç.</div><div class="egp-row" id="egpRecipients"></div><div class="egp-cats" id="egpCategories"></div><div class="egp-grid" id="egpGifts"></div><button class="egp-send" id="egpSend" type="button" disabled>Hediye gönder</button>';
  ...
  function showGiftAnimation(data){
    ...
    const sender=String(data?.sender_name||data?.sender_id||'Bir kullanıcı');
    const recipient=String(data?.recipient_name||data?.recipient_id||'bir kullanıcı');
    ...
    fx.innerHTML=`<div class="egfx-glow"></div><div class="egfx-card"><div class="egfx-level">SEVİYE ${level}</div><div class="egfx-icon">${giftIcon(name)}</div><div class="egfx-title">${esc(sender)} → ${esc(recipient)}</div><div class="egfx-gift">${esc(name)} • 💎 ${price.toLocaleString('tr-TR')}</div></div>`;
