...
function closeGate() { document.getElementById('erisGoogleGate')?.remove(); document.getElementById('erisGoogleGateStyle')?.remove(); }
      ...
      if (existing) {
        ...
        const timer = setInterval(() => {
          if (window.google?.accounts?.id) {
            ...
            clearInterval(timer);
            reject(new Error(
              "Google Identity Services 15 saniye içinde hazır olmadı."
            ));
          }
        }, 100);
  ...
  async function googleRegister() {
    ...
    let cfg;
    try {
      cfg = await request('/auth/google-config');

      if (!cfg.enabled || !cfg.client_id) {
        status.textContent = 'Google kayıt sistemi henüz etkinleştirilmemiş.';
        box.innerHTML = '<button type="button" class="authBtn authGoogle">Google ile giriş yap</button>';
      ...
      console.error('[ErisChat] Google login initialization failed:', e);
      status.textContent = e.message || 'Google giriş arayüzü yüklenemedi.';

      box.innerHTML = '<button type="button" class="authBtn authGoogle" id="erisGoogleFallback">Google ile giriş yap</button>';

      box.querySelector('#erisGoogleFallback').onclick = async () => {
        status.textContent = 'Google giriş servisi başlatılıyor…';

        try {
          const retryCfg = await request('/auth/google-config');

          if (!retryCfg.enabled || !retryCfg.client_id) {
            throw new Error('Google kayıt sistemi henüz yapılandırılmamış.');
          }

          await loadGsi();

          window.google.accounts.id.initialize({
            client_id: retryCfg.client_id,
            callback: async response => {
              try {
                status.textContent = 'Google hesabı doğrulanıyor…';
                ...
                console.error('[ErisChat] Google auth failed:', error);
                status.textContent =
                  error.message || 'Google hesabı doğrulanamadı.';
              }
            },
            auto_select: false,
            cancel_on_tap_outside: false
          ...
          status.textContent = 'Google hesabı seçme ekranı açılıyor…';
        } catch (error) {
          console.error('[ErisChat] Google retry failed:', error);
          status.textContent =
            error.message || 'Google giriş servisi başlatılamadı.';
        }
      };
    }
  }
...
async function emailOtpLogin(email, code = null) {
  ...
  function connectGeneralWs() {
    ...
    socket.onopen = () => {
      retryMs = 1000;
      emit('erischat:ws', { state: 'open' });
      socket.send(JSON.stringify({ type: 'ping' }));
    };

    socket.onmessage = event => {
      try {
        emit('erischat:event', JSON.parse(event.data));
      } catch (_) {}
    };

    socket.onclose = () => {
      emit('erischat:ws', { state: 'closed' });
  ...
  window.ErisAuth = { ensureSession, googleRegister, emailOtpLogin, logout, getToken, connectGeneralWs, getMe:()=>request('/me'), updateMe:payload=>request('/me',{method:'PATCH',body:JSON.stringify(payload)}) };
