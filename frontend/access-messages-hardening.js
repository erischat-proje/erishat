(() => {
  ...
  async function findUser(id){
    ...
    try{
      ...
      if(!box)return;
      box.innerHTML='<div class="eris-user-result" role="button" tabindex="0"><div style="display:flex;gap:10px;align-items:center"><div class="pic" style="width:42px;height:42px;display:grid;place-items:center">'+esc(user.avatar||'👤')+'</div><div><b>'+esc(user.nickname||'Kullanıcı')+'</b><span class="id">ID '+esc(user.public_id||user.id)+'</span></div></div><div class="eris-user-actions"><button type="button" data-profile>👤 Profili aç</button><button type="button" data-message>💬 Mesaj</button></div></div>';
