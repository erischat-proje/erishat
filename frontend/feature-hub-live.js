...
(() => {
  ...
  async function rooms(panel){
    ...
    try{const raw=await api('/rooms');const list=Array.isArray(raw)?raw:(raw?.rooms||raw?.items||raw?.data||[]);const wrap=panel.querySelector('#ehRoomList');wrap.innerHTML=''; if(!list.length){wrap.innerHTML='<div class="eh-note">Henüz oda yok. İlk odayı sen açabilirsin.</div>';return;} list.forEach(r=>{const row=document.createElement('div');row.className='eh-row';row.innerHTML=`<div><b>${esc(r.name)}</b><small>Lv.${r.level} • ${r.member_count}/${r.capacity} kişi • ${r.spent_lidya} Lidya</small></div>`;const controls=document.createElement('div');controls.append(btn('Katıl',async()=>{try{await api(`/rooms/${encodeURIComponent(r.id)}/join`,{method:'POST'});await roomDetail(r.id);}catch(e){window.toast?.(e.message)}}));row.append(controls);wrap.append(row);});}catch(e){panel.innerHTML+=`<div class="eh-err">Odalar yüklenemedi: ${esc(e.message)}</div>`;}
  }
  async function roomDetail(id){
    const panel=document.getElementById('eh-rooms'); try{const r=await api(`/rooms/${encodeURIComponent(id)}`);panel.innerHTML=`<div class="eh-row"><div><b>${esc(r.name)}</b><small>Seviye ${r.level} • ${r.member_count}/${r.capacity} • ${r.chat_enabled?'chat açık':'chat kapalı'}</small></div></div><div class="eh-grid"><div class="eh-card"><b>Oda sahibi</b><small>${esc(r.owner_id)}</small></div><div class="eh-card"><b>Seviye ilerlemesi</b><small>${r.spent_lidya} Lidya harcandı</small></div></div><div class="eh-kicker">Koltuklar</div><div class="eh-seats" id="ehSeats"></div><div style="display:flex;gap:7px;margin-top:9px" id="ehRoomCtl"></div><div class="eh-note" style="margin-top:9px">Oda işlemleri artık gerçek backend sistemine bağlı. Yönetici kontrolleri yalnızca yetkili kullanıcıda çalışır.</div>`;const seats=panel.querySelector('#ehSeats');const seatList=Array.isArray(r?.seats)?r.seats:(Array.isArray(r?.data?.seats)?r.data.seats:[]);seatList.forEach(s=>{const b=btn(`${s.seat_number} ${s.user_id?'●':'○'}`,async()=>{try{await api(`/rooms/${encodeURIComponent(r.id)}/seats/${s.seat_number}/join`,{method:'POST'});await roomDetail(r.id);}catch(e){window.toast?.(e.message)}});b.classList.toggle('occupied',!!s.user_id);b.disabled=!!s.locked;seats.append(b);});const ctl=panel.querySelector('#ehRoomCtl');ctl.append(btn('Koltuğu bırak',async()=>{try{await api(`/rooms/${encodeURIComponent(r.id)}/seats/leave`,{method:'DELETE'});await roomDetail(r.id);}catch(e){window.toast?.(e.message)}} ,true));ctl.append(btn('Odadan ayrıl',async()=>{try{await api(`/rooms/${encodeURIComponent(r.id)}/leave`,{method:'POST'});await rooms(panel);}catch(e){window.toast?.(e.message)}} ,true));}catch(e){panel.innerHTML=`<div class="eh-err">${esc(e.message)}</div>`;}
  }
  async function shop(panel){
    panel.innerHTML='<div class="eh-note">Görünümler canlı katalogdan geliyor. Standart avatar/çerçeveler satın alınabilir; VIP görünümler VIP seviyesine göre açılır.</div><div class="eh-grid" id="ehShopGrid">Yükleniyor…</div>';
    try{const data=await api('/cosmetics');const items=Array.isArray(data)?data:(data?.items||data?.cosmetics||data?.data||[]);const owned=await api('/me/cosmetics');const ownedItems=Array.isArray(owned)?owned:(owned?.items||owned?.cosmetics||owned?.data||[]);const ownedSet=new Set(ownedItems.map(x=>`${x.cosmetic_type}:${x.asset_key}`));const vip=await api('/me/vip');const grid=panel.querySelector('#ehShopGrid');grid.innerHTML='';items.forEach((item,i)=>{const card=document.createElement('div');card.className='eh-card';const src=window.ErisChatCosmetics?.assetUrl(item.asset_key)||item.asset_key;const vipReq=item.vip_level||0;const unlocked=!item.vip||vip.level>=vipReq;card.innerHTML=`<div class="eh-preview"><div class="eh-avatar" style="background-image:url('${src}')"></div>${item.type==='frame'?`<div class="eh-frame" style="background-image:url('${src}')"></div>`:''}</div><b>${item.type==='avatar'?'Avatar':'Çerçeve'} #${i+1}${item.vip?' • VIP':''}</b><small>${item.vip?(unlocked?`VIP ${vipReq} açıldı`:`VIP ${vipReq} gerekli`):`${item.gender||'standart'} • ${item.price} Lidya`}</small>`;const key=`${item.type}:${item.asset_key}`;if(item.vip){if(unlocked)card.append(btn('Uygula',async()=>{try{await api('/me/cosmetics/apply',{method:'POST',body:JSON.stringify({cosmetic_type:item.type,asset_key:item.asset_key})});window.toast?.('VIP görünüm uygulandı ✓');}catch(e){window.toast?.(e.message)}}));else card.append(btn('🔒 Kilitli',()=>{},true));}else if(ownedSet.has(key))card.append(btn('Uygula',async()=>{try{await api('/me/cosmetics/apply',{method:'POST',body:JSON.stringify({cosmetic_type:item.type,asset_key:item.asset_key})});window.toast?.('Görünüm uygulandı ✓');}catch(e){window.toast?.(e.message)}}));else card.append(btn(`Satın al • ${item.price}`,async()=>{try{await api('/me/cosmetics/purchase',{method:'POST',body:JSON.stringify({cosmetic_type:item.type,asset_key:item.asset_key})});await shop(panel);}catch(e){window.toast?.(e.message)}}));grid.append(card);});}catch(e){panel.innerHTML+=`<div class="eh-err">Mağaza yüklenemedi: ${esc(e.message)}</div>`;}
  }
  async function vip(panel){try{const v=await api('/me/vip');const vipData=v?.data&&typeof v.data==='object'?v.data:v;const level=Number(vipData.level||0),spent=Number(vipData.total_spent||0),thresholds=[0,1000,5000,15000,30000,60000,120000,250000,500000,1000000,2000000,5000000,10000000],next=level<12?thresholds[level+1]:null,base=thresholds[Math.min(level,12)]||0,current=vipData.current_level_spent==null?Math.max(0,spent-base):Number(vipData.current_level_spent||0),pct=next==null?100:Math.max(0,Math.min(100,(current/Math.max(1,next-base))*100));const perks=(vipData.perks||[]).map(x=>`<div class="eh-card"><b>✓ ${esc(x)}</b></div>`).join('');panel.innerHTML=`<div class="eh-note">VIP seviyeleri 1 → 12. Bu panel gerçek backend VIP verisini ve harcama ilerlemesini gösterir.</div><div class="eh-card"><b>VIP ${level}</b><small>${vipData.hidden?'Gizli':'Aktif'} • ${vipData.neon_color||'standart neon'} • ${vipData.entry_effect||'standart giriş'}</small><div style="margin-top:8px">Toplam harcama: <b>${spent.toLocaleString('tr-TR')}</b> Lidya</div>${next==null?'<small style="display:block;margin-top:4px">Maksimum VIP seviyesi</small>':`<small style="display:block;margin-top:4px">Sonraki seviye: VIP ${level+1} • ${next.toLocaleString('tr-TR')} Lidya</small>`}<div style="height:6px;background:#ffffff0b;border-radius:99px;overflow:hidden;margin-top:7px"><div style="height:100%;width:${pct}%;background:linear-gradient(90deg,#7b4cff,#ff4fa3)"></div></div></div><div class="eh-kicker">Açılan özellikler</div><div class="eh-grid">${perks||'<div class="eh-note">Henüz VIP özelliği yok.</div>'}</div>`;}catch(e){panel.innerHTML=`<div class="eh-err">${esc(e.message)}</div>`;}}
  async function family(panel){
    panel.innerHTML='<div class="eh-note">Gerçek aile davetlerini kabul et veya reddet. Aile oluşturabilir, bağış ve üyelik bilgilerini görüntüleyebilirsin.</div><div class="eh-kicker">Bekleyen davetler</div><div id="ehInvites">Yükleniyor…</div><div class="eh-kicker" style="margin-top:12px">Ailelerim</div><input id="ehFamilyName" class="eh-input" maxlength="64" placeholder="Yeni aile adı"><div id="ehFamilyCreate"></div><div id="ehFamilyList">Yükleniyor…</div><div id="ehFamilyDetail"></div>';
    const inviteBox=panel.querySelector('#ehInvites'),listBox=panel.querySelector('#ehFamilyList'),detail=panel.querySelector('#ehFamilyDetail');
    panel.querySelector('#ehFamilyCreate').append(btn('＋ Aile oluştur',async()=>{const name=panel.querySelector('#ehFamilyName').value.trim();if(!name)return window.toast?.('Aile adını yaz.');try{const f=await api('/families',{method:'POST',body:JSON.stringify({name})});localStorage.setItem('eris_family_id',f.id);await family(panel)}catch(e){window.toast?.(e.message||'Aile oluşturulamadı.')}}));
    try{
      const invitations=await api('/families/invitations');inviteBox.replaceChildren();
      if(!invitations.length)inviteBox.innerHTML='<div class="eh-note">Bekleyen aile daveti yok.</div>';
      invitations.forEach(invite=>{const row=document.createElement('div');row.className='eh-row';const title=document.createElement('span');title.textContent=(invite.family_name||'Aile')+' • '+(invite.inviter_nickname||'Kullanıcı');row.append(title);for(const [action,label] of [['accept','Kabul et'],['reject','Reddet']])row.append(btn(label,async()=>{try{await api('/families/invitations/'+encodeURIComponent(invite.id)+'/'+action,{method:'POST'});await family(panel)}catch(e){window.toast?.(e.message||'Davet işlenemedi.')}},action==='reject'));inviteBox.append(row)});
    }catch(e){inviteBox.textContent=e.message||'Aile davetleri yüklenemedi.'}
    try{
      const raw=await api('/families'),families=Array.isArray(raw)?raw:(raw?.families||raw?.items||[]);listBox.replaceChildren();
      if(!families.length){listBox.innerHTML='<div class="eh-note">Henüz üyesi olduğun aile yok.</div>';return}
      families.forEach(f=>{const b=btn((f.id===localStorage.getItem('eris_family_id')?'✓ ':'')+f.name,()=>{localStorage.setItem('eris_family_id',f.id);showFamily(f.id)},true);listBox.append(b)});
      const selected=families.find(f=>f.id===localStorage.getItem('eris_family_id'))||families[0];localStorage.setItem('eris_family_id',selected.id);await showFamily(selected.id);
    }catch(e){listBox.textContent=e.message||'Aileler yüklenemedi.'}
    async function showFamily(id){
      try{
        const [f,members]=await Promise.all([api('/families/'+encodeURIComponent(id)),api('/families/'+encodeURIComponent(id)+'/members')]);
        detail.innerHTML='<div class="eh-card" style="margin-top:8px"><b>'+esc(f.name)+'</b><small>Seviye '+Number(f.level||1)+' • kapasite '+Number(f.capacity||0)+' • bakiye '+Number(f.balance||0).toLocaleString('tr-TR')+' Lidya</small></div><div class="eh-kicker">Üyeler ('+members.length+')</div><div id="ehMembers"></div><div id="ehInviteControls"></div><input id="ehDonate" class="eh-input" type="number" min="1" max="10000000" placeholder="Lidya bağışı"><div id="ehDonateBtn"></div><div class="eh-kicker" style="margin-top:12px">Aile sohbeti</div><div id="ehFamilyMessages" style="max-height:180px;overflow:auto"></div><div style="display:flex;gap:6px"><input id="ehFamilyMessage" class="eh-input" maxlength="2000" placeholder="Aileye mesaj yaz"><button id="ehFamilySend" class="eh-btn">Gönder</button></div>';
        const memberList=detail.querySelector('#ehMembers'),meId=window.ErisAuth?.user?.id,actor=members.find(m=>m.user_id===meId),manager=actor&&['owner','admin'].includes(actor.role);members.forEach(member=>{const row=document.createElement('div');row.className='eh-row';row.textContent=(member.nickname||member.user_id)+' • '+member.role;memberList.append(row)});
        if(manager){const controls=detail.querySelector('#ehInviteControls');controls.innerHTML='<input class="eh-input" data-invite-user placeholder="Davet edilecek kullanıcı ID"><button type="button" class="eh-btn" data-invite>Üyeyi davet et</button>';controls.querySelector('[data-invite]').onclick=async()=>{const user_id=controls.querySelector('[data-invite-user]').value.trim();if(!user_id)return window.toast?.('Kullanıcı ID gir.');try{await api('/families/'+encodeURIComponent(id)+'/members',{method:'POST',body:JSON.stringify({user_id})});controls.querySelector('[data-invite-user]').value='';window.toast?.('Aile daveti gönderildi.')}catch(error){window.toast?.(error.message||'Davet gönderilemedi.')}}}
        const messages=detail.querySelector('#ehFamilyMessages');async function loadFamilyChat(){const chat=await api('/families/'+encodeURIComponent(id)+'/chat');messages.replaceChildren();for(const msg of chat.messages||[]){const item=document.createElement('div');item.className='eh-row';item.textContent=(members.find(m=>m.user_id===msg.sender_id)?.nickname||'Üye')+': '+msg.text;messages.append(item)}messages.scrollTop=messages.scrollHeight}try{await loadFamilyChat()}catch(error){messages.textContent=error.message||'Aile sohbeti yüklenemedi.'}detail.querySelector('#ehFamilySend').onclick=async()=>{const input=detail.querySelector('#ehFamilyMessage'),text=input.value.trim();if(!text)return;try{await api('/families/'+encodeURIComponent(id)+'/chat/messages',{method:'POST',body:JSON.stringify({text})});input.value='';await loadFamilyChat()}catch(error){window.toast?.(error.message||'Mesaj gönderilemedi.')}};
        detail.querySelector('#ehDonateBtn').append(btn('Bağış yap',async()=>{const amount=Number(detail.querySelector('#ehDonate').value);if(!Number.isSafeInteger(amount)||amount<1)return window.toast?.('Geçerli bağış miktarı yaz.');try{await api('/families/'+encodeURIComponent(id)+'/donate',{method:'POST',body:JSON.stringify({amount})});await showFamily(id)}catch(e){window.toast?.(e.message||'Bağış yapılamadı.')}}));
      }catch(e){detail.textContent=e.message||'Aile bilgileri yüklenemedi.'}
    }
  }
  async function discover(panel){
    ...
    const render=(title,rows,empty='Sonuç bulunamadı.')=>{
      ...
      out.innerHTML='<div class="eh-kicker">'+esc(title)+'</div>'+ (arr.length
        ? '<div class="eh-grid">'+arr.map(u=>{const uid=u.user_id||u.id||u.user?.id||'';const rid=u.room_id||u.room?.id||'';const name=u.nickname||u.display_name||u.name||u.user?.nickname||'Kullanıcı';const meta=u.distance_km!=null?String(u.distance_km)+' km':(u.room_name||u.title||u.conversation_id||rid||'Eşleşme bulundu');const action=rid&&window.openRoom?'<button class="eh-btn" data-disc-room="'+esc(rid)+'">Odayı aç</button>':uid&&window.ErisChatDM?.create?'<button class="eh-btn" data-disc-user="'+esc(uid)+'">Sohbet</button>':'';return '<div class="eh-card"><b>'+esc(name)+'</b><small>'+esc(meta)+'</small>'+action+'</div>';}).join('')+'</div>'
        : '<div class="eh-note">'+esc(empty)+'</div>');
    };
    out.addEventListener('click',e=>{const rb=e.target.closest('[data-disc-room]');if(rb){window.openRoom?.(rb.dataset.discRoom);return;}const ub=e.target.closest('[data-disc-user]');if(ub){window.ErisChatDM?.create?.(ub.dataset.discUser,ub.dataset.discUser);}});
