...
(() => {
  ...
  async function vip(panel){try{const v=await api('/me/vip');const level=Number(v.level||0),spent=Number(v.total_spent||0),thresholds=[0,1000,5000,15000,30000,60000,120000,250000,500000,1000000,2000000,5000000,10000000],next=level<12?thresholds[level+1]:null,base=thresholds[Math.min(level,12)]||0,current=v.current_level_spent==null?Math.max(0,spent-base):Number(v.current_level_spent||0),pct=next==null?100:Math.max(0,Math.min(100,(current/Math.max(1,next-base))*100));const perks=(v.perks||[]).map(x=>`<div class="eh-card"><b>✓ ${esc(x)}</b></div>`).join('');panel.innerHTML=`<div class="eh-note">VIP seviyeleri 1 → 12. Bu panel gerçek backend VIP verisini ve harcama ilerlemesini gösterir.</div><div class="eh-card"><b>VIP ${level}</b><small>${v.hidden?'Gizli':'Aktif'} • ${v.neon_color||'standart neon'} • ${v.entry_effect||'standart giriş'}</small><div style="margin-top:8px">Toplam harcama: <b>${spent.toLocaleString('tr-TR')}</b> Lidya</div>${next==null?'<small style="display:block;margin-top:4px">Maksimum VIP seviyesi</small>':`<small style="display:block;margin-top:4px">Sonraki seviye: VIP ${level+1} • ${next.toLocaleString('tr-TR')} Lidya</small>`}<div style="height:6px;background:#ffffff0b;border-radius:99px;overflow:hidden;margin-top:7px"><div style="height:100%;width:${pct}%;background:linear-gradient(90deg,#7b4cff,#ff4fa3)"></div></div></div><div class="eh-kicker">Açılan özellikler</div><div class="eh-grid">${perks||'<div class="eh-note">Henüz VIP özelliği yok.</div>'}</div>`;}catch(e){panel.innerHTML=`<div class="eh-err">${esc(e.message)}</div>`;}}
