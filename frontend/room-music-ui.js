(() => {
  ...
  async function load(){
    ...
    const r=await window.ErisRoom.music(id).catch(()=>null),rows=r&&r.ok?await r.json().catch(()=>[]):[];
  ...
  async function playback(x){
    ...
    const r=await window.ErisRoom.musicPlayback(id,x.id,action);if(!r.ok)return window.toast?.('Müzik kontrolü reddedildi');
  ...
  function mount(){
    ...
    p.querySelector('#musicAdd').onclick=async()=>{const t=musicTitle.value.trim(),u=musicUrl.value.trim();if(!t||!u)return;const r=await window.ErisRoom.addMusic(rid(),t,u);if(!r.ok)return window.toast?.('Müzik eklenemedi');musicTitle.value='';musicUrl.value='';load()};
  }
