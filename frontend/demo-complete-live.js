...
(() => {
  ...
  async function gamesDemo(scope='main', roomId=null) {
    ...
    const renderAnalytics=(g,d,out)=>{
      const rows=(d.observed_results||[]).map(x=>`<tr><td>${esc(x.key)}</td><td>${x.expected_rate}%</td><td>${x.observed_rate}%</td><td>${x.deviation_pp>0?'+':''}${x.deviation_pp}</td><td>${x.count}</td></tr>`).join('');
      out.innerHTML=`<div style="margin-top:7px;padding:9px;border-radius:10px;background:#ffffff06"><b>📊 Detaylı Analiz</b><small style="display:block;color:#938a9f;margin:4px 0">Örneklem: ${d.sample_size||0} • Son seri: ${esc(d.recent_streak?.result||'-')} ×${d.recent_streak?.length||0} • Seçim isabeti: ${d.choice_accuracy?.rate||0}%</small><div style="overflow:auto"><table style="width:100%;font-size:8px;border-collapse:collapse"><tr><th>Sonuç</th><th>Teorik</th><th>Gerçek</th><th>Sapma</th><th>Adet</th></tr>${rows}</table></div>${d.extra?.multiplier_count?`<small style="display:block;margin-top:6px">🚀 Çarpan: min ${d.extra.multiplier_min}× • max ${d.extra.multiplier_max}× • ort. ${d.extra.multiplier_average}×</small>`:''}${d.extra?.podium_counts?`<small style="display:block;margin-top:6px">🐎 Podyum: ${Object.entries(d.extra.podium_counts).map(([k,v])=>k+': '+v).join(' • ')}</small>`:''}</div>`;
    };
    async function render(kind){
      ...
      for(const g of catalog.filter(x=>x.scope===kind)){
        ...
        el.innerHTML=`<b>${esc(names[g.key]||g.key)}</b><small style="display:block;color:#938a9f;margin:5px 0 8px">${esc(g.description)}</small><div style="display:flex;gap:5px;flex-wrap:wrap"><button data-play style="border:0;border-radius:10px;background:linear-gradient(135deg,#754cff,#ff4fa3);color:#fff;padding:9px 10px;font-size:9px;font-weight:800">🎲 Oyna</button><button data-analysis style="border:1px solid #ffffff12;border-radius:10px;background:#ffffff08;color:#fff;padding:9px 10px;font-size:9px">📊 Analiz</button><button data-history style="border:1px solid #ffffff12;border-radius:10px;background:#ffffff08;color:#fff;padding:9px 10px;font-size:9px">🕘 Geçmiş</button></div><div data-out></div>`;
        const out=el.querySelector('[data-out]');
        el.querySelector('[data-play]').onclick=async()=>{if(kind==='room'&&!roomId){out.innerHTML='<small style="color:#ff9bbd">Önce bir odaya gir.</small>';return;}const res=await api('/games/'+encodeURIComponent(g.key)+'/play',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(kind==='room'?{room_id:roomId}:{})}).catch(()=>null);const d=res?await res.json().catch(()=>({})):{};out.innerHTML=`<div style="margin-top:7px;padding:9px;border-radius:10px;background:#8a5cff12">🎲 Sonuç: <b>${esc(d.result||d.detail||'Sonuç alınamadı')}</b></div>`};
        el.querySelector('[data-analysis]').onclick=async()=>{const res=await api('/games/'+encodeURIComponent(g.key)+'/analytics').catch(()=>null);const d=res?await res.json().catch(()=>({})):{};renderAnalytics(g,d,out)};
