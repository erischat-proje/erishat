...
(() => {
  ...
  async function musicDemo(){
    ...
    try { const rooms=await api('/rooms'); const arr=Array.isArray(rooms)?rooms:(rooms?.rooms||[]); roomId=arr[0]?.id||arr[0]?.room_id||''; if(roomId){ const r=await api('/rooms/'+encodeURIComponent(roomId)+'/music'); queue=await r.json(); } } catch(e){}
    ...
    const addBtn=button('➕ Kuyruğa ekle',add); controls.append(addBtn,button('🔄 Yenile',async()=>{if(roomId){const r=await api('/rooms/'+encodeURIComponent(roomId)+'/music');queue=await r.json();refresh();}}));
