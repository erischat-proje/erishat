...
(() => {
  ...
  async function familyDemo(){
    let data=null,members=[];
    try{const families=await api('/families');const rows=Array.isArray(families)?families:(families?.families||[]);data=rows[0]||null;if(data)members=await api('/families/'+encodeURIComponent(data.id)+'/members');}catch(e){}
    ...
    const render=()=>{head.innerHTML=card('<b>👑 '+esc(data?.name||'Aile bulunamadı')+'</b><small style="display:block;color:#938a9f;margin-top:4px">'+(data?('Seviye '+Number(data.level||1)+' • '+Number(data.member_count||0)+' / '+Number(data.capacity||0)+' üye • '+Number(data.balance||0).toLocaleString('tr-TR')+' bakiye'):'Hesabında bağlı aile bulunamadı')+'</small>');list.innerHTML=members.length?members.map(x=>card('<div style="display:flex;justify-content:space-between"><b>'+esc(x.nickname||x.user_id)+'</b><span>'+esc(x.role||'member')+'</span></div><small style="display:block;color:#938a9f;margin-top:4px">'+esc(x.user_id)+'</small>')).join('<div style="height:6px"></div>'):card('<small style="color:#938a9f">Üye listesi boş.</small>');};
    render();
    if(!data){actions.append(button('➕ Aile oluştur',async()=>{const name=prompt('Aile adı','ErisChat Ailesi')?.trim();if(!name)return;const d=await api('/families',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name})}).catch(e=>({detail:e.message}));if(d?.detail){window.toast?.(d.detail);return}m.remove();familyDemo();}));return;}
