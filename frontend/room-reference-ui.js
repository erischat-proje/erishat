(() => {
  ...
  async function users(body,r){const seats=Array.isArray(r?.seats)?r.seats:[],rows=seats.filter(x=>x.user_id),mods=new Set((r?.moderators||[]).map(String)),owner=isOwner(r);body.innerHTML='<div class="room-v3-note">'+rows.length+' kullanıcı koltukta • '+mods.size+'/'+Number(r?.max_moderators||0)+' moderatör.</div>'+ (rows.length?rows.map(x=>{const uid=String(x.user_id),mod=mods.has(uid),nm=esc(x.nickname||x.user_name||uid);return '<div class="room-v3-card" data-user="'+esc(uid)+'"><b>'+nm+'</b><small>Koltuk '+Number(x.seat_number||0)+' • '+(x.muted?'🔇 Susturuldu':'🎙️ Mikrofon açık')+(mod?' • 🛡️ Moderatör':'')+'</small>'+(owner?'<div class="room-v3-grid" style="margin-top:6px"><button class="room-v3-btn" data-mod="'+uid+'">'+(mod?'Moderatorsüz yap':'Moderatör yap')+'</button><button class="room-v3-btn" data-kick="'+uid+'">Odadan at</button></div>':'')+'</div>'}).join(''):'<div class="room-v3-card"><small>Koltuklarda kullanıcı yok.</small></div>');body.querySelectorAll('[data-mod]').forEach(b=>b.onclick=async()=>{try{const uid=b.dataset.mod;if(mods.has(uid))await roomApi().removeModerator?.(roomId(),uid);else await roomApi().addModerator?.(roomId(),uid);openMenu('users')}catch(e){window.toast?.(e.message||'Moderatör işlemi reddedildi')}});body.querySelectorAll('[data-kick]').forEach(b=>b.onclick=async()=>{try{await roomApi().ban?.(roomId(),b.dataset.kick);openMenu('users')}catch(e){window.toast?.(e.message||'Kullanıcı atılamadı')}})}
  async function gifts(body,r){const id=r?.id||roomId();let data=null;try{data=await roomApi().giftCatalog?.(id)}catch(e){}const gs=Array.isArray(data)?data:(data?.items||data?.gifts||[]);body.innerHTML='<div class="room-v3-card"><b>🎁 Hediye gönder</b><small>Odadaki bir kullanıcıya hediye gönderme menüsü.</small></div><div class="room-v3-grid" id="roomV3Gifts"></div>';const g=body.querySelector('#roomV3Gifts');if(!gs.length){g.innerHTML='<div class="room-v3-card"><small>Hediye kataloğu bağlantı bekliyor.</small></div>';return}gs.slice(0,24).forEach(x=>{const b=document.createElement('button');b.className='room-v3-card';b.innerHTML='<b>'+esc(x.emoji||x.icon||'🎁')+' '+esc(x.name||x.title||x.key||'Hediye')+'</b><small>'+Number(x.price||x.cost||0).toLocaleString('tr-TR')+' Lidya</small>';b.onclick=()=>window.toast?.('Hediye seçildi ✓');g.appendChild(b)})}
  async function music(body,r){const id=r?.id||roomId();let data=[];try{data=await roomApi().music?.(id)||[]}catch(e){}const rows=Array.isArray(data)?data:(data?.items||data?.music||[]);body.innerHTML='<div class="room-v3-card"><b>🎵 Oda müziği</b><small>Ortak müzik kuyruğu oda içinde açılır.</small></div>'+ (rows.length?rows.map(x=>'<div class="room-v3-card"><b>'+esc(x.title||'Müzik')+'</b><small>'+(x.is_playing?'▶ Oynuyor':'⏸ Bekliyor')+'</small></div>').join(''):'<div class="room-v3-card"><small>Kuyruk boş.</small></div>')}
  async function settings(body,r){const owner=isOwner(r);body.innerHTML='<div class="room-v3-card"><b>⚙️ Oda ayarları</b><small>'+(r?.locked?'🔒 Oda kilitli':'🟢 Oda açık')+' • '+(r?.chat_enabled===false?'Sohbet kapalı':'Sohbet açık')+'</small></div><div class="room-v3-grid"><button class="room-v3-btn" id="roomV3Lock">'+(r?.locked?'Kilidi aç':'Odayı kilitle')+'</button><button class="room-v3-btn" id="roomV3Chat">'+(r?.chat_enabled===false?'Sohbeti aç':'Sohbeti kapat')+'</button></div>'+(owner?'<div class="room-v3-card" style="margin-top:7px"><b>👑 Oda sahibi</b><small>Oda adı ve gelişmiş kontroller sana ait.</small><button class="room-v3-btn primary" id="roomV3Rename" style="margin-top:7px">Oda adını değiştir</button></div>':'');
...
(() => {
  ...
  function seatActions(){const box=q('erisLiveSeats');if(!box||box.dataset.v5SeatsBound==='1')return;box.dataset.v5SeatsBound='1';box.addEventListener('click',async e=>{const seat=e.target.closest('.eris-seat');if(!seat||seat.classList.contains('empty'))return;const num=Number(seat.dataset.seatNumber||0),name=seat.querySelector('b')?.textContent||'Kullanıcı',id=rid(),demoMode=id.startsWith('demo-room-');const choice=prompt(name+' • Koltuk '+num+'\n1 = Sessize al/aç\n2 = Koltuğu kilitle/aç\n3 = Odadan at','');if(choice==='1'){try{if(!demoMode){const muted=seat.classList.contains('seat-muted');if(muted){await window.ErisRoom.unmuteSeat(id,num);seat.classList.remove('seat-muted')}else{await window.ErisRoom.muteSeat(id,num);seat.classList.add('seat-muted')}}else{seat.dataset.muted=seat.dataset.muted==='1'?'0':'1';seat.querySelector('.seat-mic').textContent=seat.dataset.muted==='1'?'🔇':'🎙'}window.toast?.('Koltuk sesi ayarı güncellendi ✓')}catch(e){window.toast?.(e.message||'Susturma reddedildi')}}else if(choice==='2'){try{if(!demoMode){if(seat.classList.contains('locked')){await window.ErisRoom.unlockSeat(id,num);seat.classList.remove('locked')}else{await window.ErisRoom.lockSeat(id,num);seat.classList.add('locked')}}else seat.classList.toggle('locked');window.toast?.('Koltuk kilidi güncellendi ✓')}catch(e){window.toast?.(e.message||'Koltuk kilitlenemedi')}}else if(choice==='3'){try{if(!demoMode){await window.ErisRoom.ban(id,seat.dataset.userId||'')}const rows=JSON.parse(localStorage.getItem('eris_demo_ban_history_'+id)||'[]');rows.unshift({name,reason:'Odadan atıldı',time:new Date().toLocaleString('tr-TR')});localStorage.setItem('eris_demo_ban_history_'+id,JSON.stringify(rows.slice(0,50)));seat.classList.remove('occupied');seat.classList.add('empty');seat.querySelector('b').textContent='Koltuk '+num;seat.querySelector('small').textContent='Boş • otur';seat.querySelector('.seat-ava').textContent='＋';window.toast?.(name+' odadan atıldı ✓')}catch(e){window.toast?.(e.message||'Kullanıcı atılamadı')}}})}
