(() => {
  ...
  async function users(body,r){const seats=Array.isArray(r?.seats)?r.seats:[];const rows=seats.filter(x=>x.user_id);body.innerHTML='<div class="room-v3-note">'+rows.length+' kullanıcı koltukta.</div>'+ (rows.length?rows.map(x=>'<div class="room-v3-card"><b>'+esc(x.nickname||x.user_name||'Kullanıcı')+'</b><small>Koltuk '+Number(x.seat_number||0)+' • '+(x.muted?'🔇 Susturuldu':'🎙️ Mikrofon açık')+'</small></div>').join(''):'<div class="room-v3-card"><small>Koltuklarda kullanıcı yok.</small></div>')}
