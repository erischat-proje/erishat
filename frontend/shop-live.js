...
(() => {
  ...
  async function render() {
    ...
    try {
      ...
      if (!items.length) {
        grid.innerHTML = '<div class="liveShopNote">Şu anda satışa açık kozmetik bulunmuyor. Katalog hazır olduğunda ürünler burada otomatik görünecek.</div>';
        return;
      }
      ...
      const renderItems = filter => {
        ...
        filtered.forEach((item, index) => {
          ...
          if (isVip && !unlocked) {
            ...
            action.onclick = async () => { try { await api('/me/cosmetics/apply', {method:'POST', body:JSON.stringify({cosmetic_type:type, asset_key:key})}); } catch (error) {
          window.toast?.('Görünüm şu anda uygulanamıyor.');
          return;
        } window.ErisChatCosmetics?.load(); window.toast?.('Görünüm uygulandı ✓'); };
          } else {
            const price = Number(item.price || 1000);
            action.textContent = `Satın al • ${price.toLocaleString('tr-TR')}`;
            action.onclick = async () => { const price = Number(item.price || catalog.price || 1000); try { await api('/me/cosmetics/purchase', {method:'POST', body:JSON.stringify({cosmetic_type:type, asset_key:key})}); } catch (error) {
          window.toast?.('Satın alma şu anda kullanılamıyor.');
          return;
        } await render(); window.toast?.('Kozmetik satın alındı ✓'); };
          }
      ...
      grid.innerHTML = '<div class="liveShopNote">Mağaza şu anda kullanılamıyor. Lütfen biraz sonra tekrar dene.</div>';
      console.warn('[ErisChat] shop unavailable', error);
