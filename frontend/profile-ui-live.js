(() => {
  ...
  const boot = () => {
    ...
    const roomsBtn=document.createElement('button');roomsBtn.type='button';roomsBtn.textContent='🏠 Odalarım';roomsBtn.style.cssText='border:1px solid #ffffff14;background:#ffffff05;color:#fff;border-radius:12px;padding:11px;font-size:10px;font-weight:800';controls.insertBefore(roomsBtn,logoutButton||null);
    const openMyRooms=async()=>{const modal=document.createElement('div');modal.style.cssText='position:fixed;inset:0;z-index:500;background:rgba(2,1,7,.78);backdrop-filter:blur(10px);display:grid;place-items:center;padding:18px';modal.innerHTML='<div style="width:min(440px,100%);max-height:82vh;overflow:auto;background:#0b0911;border:1px solid #ffffff14;border-radius:24px;padding:18px;color:#fff"><div style="display:flex;justify-content:space-between;align-items:center"><b>🏠 Odalarım</b><button class="close" data-close>×</button></div><div data-myrooms style="margin-top:12px">Yükleniyor…</div></div>';document.body.appendChild(modal);modal.querySelector('[data-close]').onclick=()=>modal.remove();try{const rows=await window.ErisPlatform.api('/rooms/me/rooms');const box=modal.querySelector('[data-myrooms]');box.innerHTML=rows.length?rows.map(r=>'<button data-room="'+String(r.id).replace(/"/g,'&quot;')+'" data-name="'+String(r.name||'Oda').replace(/"/g,'&quot;')+'" style="width:100%;text-align:left;margin-bottom:8px;border:1px solid #ffffff12;background:#ffffff06;color:#fff;border-radius:16px;padding:12px"><b>🏠 '+String(r.name||'Oda')+'</b><small style="display:block;color:#938a9f;margin-top:4px">'+(r.role==='owner'?'👑 Oda sahibi':'🛡️ Moderatör')+' • ID '+String(r.public_id||r.id)+' • Seviye '+Number(r.level||1)+'</small></button>').join(''):'<div style="color:#938a9f;font-size:9px">Sahibi veya moderatörü olduğun oda yok.</div>';box.querySelectorAll('[data-room]').forEach(b=>b.onclick=()=>{modal.remove();window.openRoom?.(b.dataset.room,b.dataset.name)});}catch(e){modal.querySelector('[data-myrooms]').textContent=e.message||'Odalar alınamadı.'}};roomsBtn.onclick=openMyRooms;
