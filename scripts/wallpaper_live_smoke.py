...
if len(normal)!=12 or len(vip)!=12: raise AssertionError(f"expected 12+12 wallpapers, got {len(normal)}+{len(vip)}")
for x in items:
    if not x.get("asset_key"): raise AssertionError(f"missing asset key: {x}")
...
if s>=300 or after.get("asset_key")!=item["asset_key"]: raise AssertionError(f"wallpaper persistence failed: {s} {after}")
...
if claim_key != "vip_wallpaper_10": raise AssertionError(f"VIP10 wallpaper key mismatch: {claim_key}")
