...
async function main(){
  ...
  const rtcSignalingRealtime=await page.evaluate(async ({roomId,ownerToken,memberToken,ownerId,memberId})=>{
    ...
    const ownerReady=await wait('owner RTC ready',owner.messages,m=>m?.type==='rtc_ready');
    const memberReady=await wait('member RTC ready',member.messages,m=>m?.type==='rtc_ready');
    const resolvedOwnerId=String(ownerReady.user_id||ownerId);
    const resolvedMemberId=String(memberReady.user_id||memberId);
    if(resolvedOwnerId!==String(ownerId)) throw new Error('owner RTC identity mismatch: expected='+ownerId+' ready='+resolvedOwnerId);
    if(resolvedMemberId!==String(memberId)) throw new Error('member RTC identity mismatch: expected='+memberId+' ready='+resolvedMemberId);
    member.ws.send(JSON.stringify({type:'rtc_offer',to_user_id:resolvedOwnerId,payload:{type:'offer',sdp:'browser-smoke-offer'}}));
    const receivedOffer=await wait('offer',owner.messages,m=>m?.type==='rtc_offer'&&m.from_user_id===resolvedMemberId&&m.payload?.sdp==='browser-smoke-offer');
    owner.ws.send(JSON.stringify({type:'rtc_answer',to_user_id:resolvedMemberId,payload:{type:'answer',sdp:'browser-smoke-answer'}}));
    const receivedAnswer=await wait('answer',member.messages,m=>m?.type==='rtc_answer'&&m.from_user_id===resolvedOwnerId&&m.payload?.sdp==='browser-smoke-answer');
    member.ws.send(JSON.stringify({type:'rtc_ice',to_user_id:resolvedOwnerId,payload:{candidate:'browser-smoke-ice'}}));
    const receivedIce=await wait('ice',owner.messages,m=>m?.type==='rtc_ice'&&m.from_user_id===resolvedMemberId&&m.payload?.candidate==='browser-smoke-ice');
