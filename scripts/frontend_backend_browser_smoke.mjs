...
async function main(){
  ...
  const rtcSignalingRealtime=await page.evaluate(async ({roomId,ownerToken,memberToken,ownerId,memberId})=>{
    ...
    await wait('owner history',owner.messages,m=>m?.type==='room_history'); await wait('member history',member.messages,m=>m?.type==='room_history'); await wait('owner RTC ready',owner.messages,m=>m?.type==='rtc_ready'&&m.user_id===ownerId); await wait('member RTC ready',member.messages,m=>m?.type==='rtc_ready'&&m.user_id===memberId);
