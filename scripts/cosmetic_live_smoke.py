...
def main() -> int:
    ...
    expected = {"avatar": 71, "frame": 40, "vip_avatar": 24, "vip_frame": 12}
    if counts != expected or len(items) != 147:
        raise RuntimeError(f"catalog inventory mismatch: counts={counts}, total={len(items)}, expected={expected}, total=147")
