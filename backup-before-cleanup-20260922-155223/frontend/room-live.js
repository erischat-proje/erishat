(() => {
  ...
  window.addEventListener('erischat:-rooms-fallback',()=>{
    const data=[
      ['-room-1','Gece Muhabbeti',8,'Rana','🌙',1],
      ['-room-2','Müzik Köşesi',12,'Eris','🎵',5],
      ['-room-3','Yeni Tanışmalar',5,'Lavin','💜',1],
      ['-room-4','Oyun Salonu',9,'Noir','🎮',5],
      ['-room-5','VIP Lounge',11,'NØXIA','👑',7],
      ['-room-6','Gece Yayını',7,'Mira','✨',7]
    ];
    window.ErisDemoRoomConfig=Object.fromEntries(data.map(r=>[r[0],{id:r[0],name:r[1],member_count:r[2],owner:r[3],level:r[5],seat_count:r[5]>=7?16:r[5]>=5?12:8,locked:r[0]==='-room-5',password:r[0]==='-room-5'?'3456':''}]));
    document.querySelectorAll('#realRooms,#rooms').forEach(el=>{el.innerHTML='';data.forEach(r=>{const b=document.createElement('button');b.className='room card';b.innerHTML='<div class="ava">'+r[4]+'<span class="online"></span></div><div class="grow roomText"><b></b><small></small></div><span class="live">CANLI</span>';b.querySelector('b').textContent=r[1];b.querySelector('small').textContent=r[2]+' kişi • '+r[3];b.onclick=()=>openRoom(r[0],r[1]);el.appendChild(b)});});
  });
  ...
  async function openRoom(roomId,name){
    ...
    document.getElementById('erisLiveMeta').textContent=id.startsWith('-room-')?'ID: 482731 • DEMO':'Gerçek oda • bağlanıyor…';
    ...
    // freezing the customer  while opening an example room.
    if(id.startsWith('-room-')){
      window.__erisRoomPermissions={is_owner:false,is_moderator:false,can_manage:false,current_user_seat:null};
      const =window.ErisDemoRoomConfig?.[id]||{id,name:name||'Demo Oda',level:1,seat_count:8,member_count:1,owner:'ErisChat',locked:false,password:''};
      if(id==='-room-5' && localStorage.getItem('eris__lock_v2_initialized')!=='1'){
        localStorage.setItem('eris__room_locked_'+id,'1');
        localStorage.setItem('eris__room_password_'+id,'3456');
        localStorage.setItem('eris__lock_v2_initialized','1');
      }
      const defaultLocked=.locked===true;
      const savedLocked=localStorage.getItem('eris__room_locked_'+id);
      const locked=savedLocked===null?defaultLocked:savedLocked==='1';
      const password=localStorage.getItem('eris__room_password_'+id)||String(.password||'');
      if(locked && password){
        const unlocked=localStorage.getItem('eris__room_unlocked_'+id)==='1';
        if(!unlocked){
          let accepted=false;
          ...
          localStorage.setItem('eris__room_unlocked_'+id,'1');
          window.toast?.('Şifre doğru ✓ Demo odaya giriş kabul edildi.');
        }
      }
      const seatCount=Math.min(16,Math.max(8,Number(localStorage.getItem('eris__room_capacity_'+id)||.seat_count)||8));
      const SavedName=localStorage.getItem('eris__room_name_'+id)||.name||name||'Demo Oda';
      document.getElementById('erisLiveTitle').textContent=SavedName;
      document.getElementById('erisLiveMeta').textContent='ID: 482731 • DEMO';
      document.getElementById('erisLiveMeta').dataset.roomNameMeta='ID: 482731 • DEMO';
      const levelButton=document.getElementById('erisRoomLevel');
      if(levelButton)levelButton.innerHTML='<b>Seviye '+Number(.level||1)+'</b><small>'+seatCount+' koltuk</small>';
      const seats=Array.from({length:seatCount},(_,i)=>({seat_number:i+1,user_id:i===0?'-owner':null,nickname:i===0?(.owner||'Oda Sahibi'):''}));
      renderRoomSeats(id,SavedName,seats,seatCount);
      const list=document.getElementById('erisLiveChat');
      if(list)list.innerHTML='<div class="eris-chat-msg"><b>'+String(.owner||'Oda Sahibi')+'</b><span>Hoş geldiniz 👋</span></div><div class="eris-chat-msg"><b>ErisChat</b><span>'+seatCount+' koltuklu  oda hazır.</span></div>';
      window.dispatchEvent(new CustomEvent('erischat:room-opened',{detail:{room:{...,id,name:SavedName,seat_count:seatCount}}}));
      return;
    }
  ...
  // Customer  surfaces are part of the real room entrypoint so the firm- integrity check
  // and the room UI use the same loaded surface rather than a hidden/duplicate  bootstrap.
  ['./full--live.js','./-complete-live.js','./-extras-live.js'].forEach(file => {
    if (document.querySelector('script[data-eris-="'+file+'"]')) return;
