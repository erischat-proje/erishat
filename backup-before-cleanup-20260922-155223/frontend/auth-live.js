...
function closeGate() { document.getElementById('erisGoogleGate')?.remove(); document.getElementById('erisGoogleGateStyle')?.remove(); }
  ...
  async function googleRegister() {
    ...
    try {
      ...
      window.google.accounts.id.initialize({
        ...
        callback: async response => {
          ...
          try {
            ...
            status.textContent =
              e.message || 'Google kaydı başarısız.';
          }
        },
        auto_select: false,
      ...
      status.textContent = 'Google ile giriş yapabilirsiniz.';
    } catch (e) {
      console.error('[ErisChat] Google initialization failed:', e);
      status.textContent =
        e.message || 'Google giriş servisi başlatılamadı.';
      box.innerHTML = '';
    }
  }
