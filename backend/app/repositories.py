...
class ConversationRepository:
    ...
    def find_direct(self, user_ids: list[str]) -> Conversation | None:
        ...
        stmt = (select(Conversation)
                ...
                .having(select(ConversationMember.user_id)
                       .where(ConversationMember.conversation_id == Conversation.id)
                       .where(ConversationMember.user_id.in_(user_ids))
                       .correlate(Conversation).scalar_subquery().is_not(None)))
        for conversation in self.db.scalars(stmt).unique():
            members = set(self.members(conversation.id))
