...
def family_payload(db: Session, family: Family) -> dict:
    ...
    return {
        ...
        "level": level,
        "capacity": FAMILY_LEVELS[level]["capacity"],
        "member_count": count,
        "chat_conversation_id": family.chat_conversation_id,
    }


@router.get("/families/{family_id}")
def get_family_details(family_id: str, db: Session = Depends(get_db), user: User = Depends()):
    family = get_family(db, family_id)
    membership(db, family_id, user.id)
    return family_payload(db, family)


@router.post("/families", status_code=201)
def create_family(payload: FamilyCreate, db: Session = Depends(get_db), user: User = Depends()):
    ...
    db.add(family)
    db.add(FamilyMember(family_id=family_id, user_id=user.id, role="owner"))
    db.commit()
    db.refresh(family)
    return family_payload(db, family)
...
def list_members(family_id: str, db: Session = Depends(get_db), user: User = Depends()):
    ...
    for row in rows:
        ...
        if not target:
            continue
        result.append({"user_id": row.user_id, "nickname": target.nickname, "avatar": target.avatar, "role": row.role, "joined_at": row.created_at})
    return result


@router.post("/families/{family_id}/members", status_code=201)
def invite_member(family_id: str, payload: FamilyMemberUpdate, db: Session = Depends(get_db), user: User = Depends()):
    family = get_family(db, family_id)
    actor = membership(db, family_id, user.id)
    ...
    db.add(row)
    db.add(ConversationMember(conversation_id=family.chat_conversation_id, user_id=target.id))
    db.commit()
...
def update_member_role(family_id: str, member_user_id: str, payload: FamilyMemberUpdate, db: Session = Depends(get_db), user: User = Depends()):
    family = get_family(db, family_id)
    actor = membership(db, family_id, user.id)
    if not can_manage(family, actor):
        raise HTTPException(status_code=403, detail="Rol değiştirmek için aile yöneticisi olmalısınız")
    if payload.user_id != member_user_id or payload.role not in {"member", "admin"}:
        raise HTTPException(status_code=400, detail="Geçerli rol gerekli")
    target = membership(db, family_id, member_user_id)
    if family.owner_id == member_user_id:
        raise HTTPException(status_code=400, detail="Aile sahibinin rolü değiştirilemez")
...
def remove_member(family_id: str, member_user_id: str, db: Session = Depends(get_db), user: User = Depends()):
    family = get_family(db, family_id)
    actor = membership(db, family_id, user.id)
    if not can_manage(family, actor):
        raise HTTPException(status_code=403, detail="Üye çıkarmak için aile yöneticisi olmalısınız")
    if family.owner_id == member_user_id:
        raise HTTPException(status_code=400, detail="Aile sahibi çıkarılamaz")
    target = membership(db, family_id, member_user_id)
    db.delete(target)
    ...
    db.commit()
    return {"family_id": family_id, "user_id": member_user_id, "removed": True}


@router.post("/families/{family_id}/donate")
def donate_family(family_id: str, payload: FamilyDonationCreate, db: Session = Depends(get_db), user: User = Depends()):
    family = get_family(db, family_id)
    membership(db, family_id, user.id)
    if user.lidya < payload.amount:
        raise HTTPException(status_code=400, detail="Yetersiz Lidya")
    user.lidya -= payload.amount
    family.balance += payload.amount
    family.level = family_level(family.balance)
    db.add(FamilyDonation(family_id=family.id, user_id=user.id, amount=payload.amount))
    db.commit()
    return family_payload(db, family)


@router.get("/families/{family_id}/chat")
def family_chat(family_id: str, limit: int = Query(100, ge=1, le=200), offset: int = Query(0, ge=0), db: Session = Depends(get_db), user: User = Depends()):
    family = get_family(db, family_id)
    membership(db, family_id, user.id)
...
def send_family_message(family_id: str, payload: FamilyMessageCreate, db: Session = Depends(get_db), user: User = Depends()):
    family = get_family(db, family_id)
    membership(db, family_id, user.id)
    text = payload.text.strip()
    message = Message(conversation_id=family.chat_conversation_id, sender_id=user.id, text=text)
    db.add(message)
    db.commit()
    db.refresh(message)
    return message
